...
public:
	...
	{//To insert values in tree
		...
		{//If tree is empty newnode is made as root
			...
			if (ploc->ID > value)
			{
				...
				if (!isbalanced(root))
				{
					cout << "node disturbs avial condition" << endl;
					if (leftheavy)
					{
						balanceFactor(root->leftchild);
						if (leftheavy)
						{
							cout << "leftleftheavy" << endl;
							RotateforLeft(root);
						}
						else
						{
							cout << "leftrightheavy" << endl;
				...
				else { cout << endl << "Tree is  balanced" << endl; }
			}
			else {
				cout << "rightinsertion" << endl;
				ploc->rightchild = nn;
	...
	void RotateforRight(Node* x)
	{
		...
		{//To restore the connection with the previous node
			/*while (loc->data!= x->data)
			{//Basically it is a code of search method so that we can have location and parent of loc
				if (loc->data > x->data)
				{
					ploc = loc;
					loc = loc->leftchild;
...
int main() {
	...
	endl << "2 for deleting student from record" << endl << "3 for seeing all student names and ID" << endl;
...
//cout<<endl<<"Height is: "<<a->Height(a->root);
//a->DeleteSubTree(5);

//cout << "PreOrderTraversal After Deletion";
//a->PreOrderTraversal(a->root);
//cout<<endl << a->root->data;
